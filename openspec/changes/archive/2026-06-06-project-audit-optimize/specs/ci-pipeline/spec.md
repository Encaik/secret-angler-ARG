...
The project SHALL have `ci`, `lint`, `lint:fix`, and `format` scripts defined in `package.json`.

#### Scenario: Full CI check runs locally

- **WHEN** `pnpm ci` is executed
...
- **WHEN** comparing `pnpm ci` output to the GitHub Actions workflow
