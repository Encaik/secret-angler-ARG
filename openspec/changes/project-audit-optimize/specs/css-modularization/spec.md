...
All 11 existing inline `<style>` blocks in `.astro` files SHALL be either migrated to the appropriate global CSS file or confirmed as intentional page-specific scoped styles.
...
- **THEN** any remaining `<style>` blocks are Astro scoped styles (`<style lang="css">` with component-specific selectors) or are explicitly documented as intentional
